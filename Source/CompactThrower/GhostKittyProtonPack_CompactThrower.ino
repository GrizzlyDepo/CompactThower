...
/****************************************************************************************
  ...
  2024 - GrizzlyDepo changes - Arm Thrower (AKA Single Arm Ghost Entangler or SAGE)
  - changed the LED pattern to remove vent and orange hat lights
  - change the GBLED Patterns to redcuce ring light to 12 lights and changes 'speed' of ring light for afterlife
  - updated code due to less lights in LED string for wand.
******************************************************************************************/
...
//const int ventStart = 0;  GrizzlyDepo - removed vent lights and hat light and renumbered LEDS
//const int ventEnd = 3;
const int SloBloLED = 0;
const int WandVentLED = 1;
const int ThemeLED = 2;
const int WhiteLED1 = 3;
const int WhiteLED2 = 4;
//const int OrangeHatLED = 9;
const int GunLEDStart = 5;
const int GunLEDEnd = 11;
...
const int NeoPixelLEDCount1 = 12;

// Vent + Wand LED Count
const int NeoPixelLEDCount2 = 12;
  ...
  if (WANDSTATUS == ON_PACK) // Turn on the Proton Pack if both the wand activate switch and Pack switch are ON
  {
    ...
    switch (STATUS)
    {
      ...
      case BOOTING:
        ...
        setWandLightState(0, 7, currentMillis); //set sloblow red blinking
...
/* Wand LED assighment - GrizzlyDepo Update for Compact Thower
  const int SloBloLED = 0;
  const int WandVentLED = 1;
  const int ThemeLED = 2;
  const int WhiteLED1 = 3;
...
void WandLightState(unsigned long currentMillis)
{
  switch (WANDLEDSTATUS)  // enum WandSLEDState { ALLOFF, WANDONLY, NORMAL, FIRING, WARNING, FASTWARNING, VENTSTATE };
  {
    ...
    case WANDONLY:
      ...
      //setVentLightState(ventStart, ventEnd, 2); GrizzlyDepo - commented out 
    ...
    case NORMAL: // Wand is ON and Pack is ON
      ...
      //setWandLightState(OrangeHatLED, 4, 0); // GrizzlyDepo - commented out 
      //setVentLightState(ventStart, ventEnd, 2); GrizzlyDepo - commented out 
    ...
    case FASTWARNING:
      ...
      break;
    case VENTSTATE:
      //setVentLightState(ventStart, ventEnd, 0); GrizzlyDepo - commented out 
     // setWandLightState(OrangeHatLED, 4, 0);  GrizzlyDepo - commented out 
      setWandLightState(SloBloLED, 4, 0);    //  set sloblo  led OFF
      clearGunLEDs(); //Turn off Gun LED's
      break;
...
int fireSeqNum = 0;
int fireSeqTotal = 5;
//GrizzlyDepo - had to change the case since firing LEDs are 5 through 11 now
void fireStrobe(unsigned long currentMillis) {
  if ((unsigned long)(currentMillis - prevFireMillis) >= fire_interval2) {
    ...
    switch ( fireSeqNum ) {
      case 0:
        wandLights.setPixelColor(5, wandLights.Color(ThemeGunColors1[THEME], ThemeGunColors2[THEME], ThemeGunColors3[THEME]));
        ...
        break;
      case 1:
        wandLights.setPixelColor(5, wandLights.Color(ThemeGunColors13[THEME], ThemeGunColors23[THEME], ThemeGunColors33[THEME]));
        ...
        break;
      case 2:
        wandLights.setPixelColor(5, wandLights.Color(ThemeGunColors12[THEME], ThemeGunColors22[THEME], ThemeGunColors32[THEME]));
        wandLights.setPixelColor(6, 0);
        wandLights.setPixelColor(7, wandLights.Color(ThemeGunColors13[THEME], ThemeGunColors23[THEME], ThemeGunColors33[THEME]));
      ...
      case 4:
        ...
        break;
      case 5:
        wandLights.setPixelColor(5, wandLights.Color(ThemeGunColors14[THEME], ThemeGunColors24[THEME], ThemeGunColors34[THEME]));
        wandLights.setPixelColor(6, wandLights.Color(ThemeGunColors15[THEME], ThemeGunColors25[THEME], ThemeGunColors35[THEME]));
        ...
        wandLights.setPixelColor(9, wandLights.Color(ThemeGunColors14[THEME], ThemeGunColors24[THEME], ThemeGunColors34[THEME]));
        wandLights.setPixelColor(10, wandLights.Color(ThemeGunColors1[THEME], ThemeGunColors2[THEME], ThemeGunColors3[THEME]));
        wandLights.setPixelColor(11, wandLights.Color(ThemeGunColors13[THEME], ThemeGunColors23[THEME], ThemeGunColors33[THEME]));
        break;
    }
